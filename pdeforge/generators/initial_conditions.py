...
class ICGenerator(ABC):
    ...
    @abstractmethod
    def generate(self, shape: Tuple[int, ...], seed: int = None) -> np.ndarray:
        """
    ...
    def _generate_2d(
        ...
        for kx in range(1, self.n_modes + 1):
            for ky in range(1, self.n_modes + 1):
                ...
                # Sin-Sin
                a = np.random.randn() * decay_factor * self.amplitude
                u += a * np.sin(2 * np.pi * kx * X / Lx) * np.sin(2 * np.pi * ky * Y / Ly)
                
                if self.use_cos:
                    # Sin-Cos
                    b = np.random.randn() * decay_factor * self.amplitude
                    u += b * np.sin(2 * np.pi * kx * X / Lx) * np.cos(2 * np.pi * ky * Y / Ly)
                    
                    # Cos-Sin
                    c = np.random.randn() * decay_factor * self.amplitude
                    u += c * np.cos(2 * np.pi * kx * X / Lx) * np.sin(2 * np.pi * ky * Y / Ly)
                    
                    # Cos-Cos
                    d = np.random.randn() * decay_factor * self.amplitude
                    u += d * np.cos(2 * np.pi * kx * X / Lx) * np.cos(2 * np.pi * ky * Y / Ly)
...
class GaussianRandomFieldGenerator(ICGenerator):
    ...
    def generate(
        ...
        if ndim == 1:
            ...
            return self._generate_2d(shape)
        else:
            raise ValueError(f"Unsupported dimensionality: {ndim}")
...
def get_ic_generator(
    ...
    **params
    ...
    if name not in generators:
        raise ValueError(f"Unknown IC generator: {name}. Available: {list(generators.keys())}")
