...
def explore_parameter(
    ...
    if (
        param_name not in user_param_names
        and param_name not in model_cls.DEFAULT_PARAMS
    ):
        raise ValueError(
...
def _visualize_1d_parameter_effect(
    ...
    for ic_idx in range(n_per_value):
        ...
        for val_idx, val in enumerate(unique_values):
            ...
            axes[ic_idx, 1].plot(
                x,
                dataset.outputs[sample_idx],
                color=colors[val_idx],
                linewidth=2,
                label=label,
            )
...
def _visualize_2d_parameter_effect(
    ...
    for val_idx, val in enumerate(unique_values):
        ...
        im0 = axes[0, val_idx].pcolormesh(
            x, y, input_viz.T, shading="auto", cmap="viridis"
        )
        axes[0, val_idx].set_title(f"{param_name}={val:.2g}\nInput")
        axes[0, val_idx].set_aspect("equal")
        plt.colorbar(im0, ax=axes[0, val_idx])

        im1 = axes[1, val_idx].pcolormesh(
            x, y, output_viz.T, shading="auto", cmap="RdBu_r"
        )
